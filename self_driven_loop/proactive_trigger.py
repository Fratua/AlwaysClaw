...
class ProactiveTriggerEngine:
    ...
    def _get_historical_accuracy(self, pattern: TriggerPattern) -> float:
        """Get historical accuracy of pattern from trigger history."""
        relevant = [t for t in self.trigger_history if t.pattern.name == pattern.name]
        if not relevant:
            return 0.75  # Prior for unseen patterns
        successful = sum(1 for t in relevant if getattr(t, 'was_successful', True))
